	...
	<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
		<div class="container">
			<ul class="nav navbar-nav">
				<li role="presentation" class="active"><a href="index_monitor">实时监控</a></li>
				<li role="presentation"><a href="index_runtime">系统参数</a></li>
				<li role="presentation"><a href="index_thread">线程信息</a></li>
			</ul>
		</div>
	</nav>
	<div class="container">
		...
		$(function() {
			...
			nodeSelectedFunction = function(event, node) {
				if (node['cluster']) {
					app = node['id'];
					updateURL(node);
					MonitorServer.loadData(node['cluster'], app);
				}
			}
		});
	</script>
