...
function updateURL(node) {
    var tips = $("#app");
    if (tips) {
        tips.text(String.format("%s:(%s-[%s:%s])", MONITOR, node.text, node.host, node.port))
    }
}

function updateTitle(appName, appHost, appPort) {
    var tips = $("#app");
    if (tips) {
        tips.text(String.format("%s:(%s-[%s:%s])", MONITOR, appName, appHost, appPort))
    }
}
...
function handleAjaxErr(jq, status, err) {
    Timer.stop();
    if (jq.responseText) alert(jq.responseText);
    else if (err) alert(err);
    else alert(jq.status)
}

function beforeAjaxSend(jq) {
    if (msgUrl == this.url) return true;
    if (clsUrl == this.url) return true;
    if (!checkHasSelectApp()) return false;
    if (this.data) {
        this.data["app"] = app
...
Timer = {
    ...
    stop: function () {
        var id;
        var ids = this.ids;
        while (id = ids.shift()) clearTimeout(id)
    }
};
LongPolling = {
    listener: [],
...
MonitorServer = {
    ...
    updateGCLine: function (time, gc) {
        ...
        for (i in series) {
            ...
            var key = ser.name;
            var item = gc[key];
            if (!lastgc) ser.addPoint([time, item], true, shift);
            else ser.addPoint([time, item - lastgc[key]], true, shift);
            if (key.indexOf("count") > 0) count += item;
            else if (key.indexOf("time") > 0) times += item
        }
        lastgc = gc;
        $("#gcTitle").html(String.format("GC[%s:%s %s:%sms]", GC_TOTAL, count, GC_TIMES, Math.round(times * 1e3)))
    },
    updateCpuLine: function (time, cpu) {
        ...
        var title = String.format("OS:%s% JVM:%s% Core:%s %s:%sMB", cpu.os, cpu.vm, cpu.cores, FREEMEMORY, momory);
        $("#cpuTitle").html(title)
    },
    updateHeapArea: function (time, heap) {
        ...
        $("#heapTitle").text(this.buildTitle("Heap", heap.max, heap.used, heap.init))
    },
    updateNonHeapArea: function (time, nonheap) {
        ...
        $("#edenTitle").text(this.buildTitle("Nonheap", nonheap.max, nonheap.used, nonheap.init))
    },
    buildTitle: function (name, max, used, init) {
        ...
        return String.format("%s-Max:%sMB Init:%sMB,Used:%sMB", name, max, init, use)
    },
    doGC: function (obj) {
...
ThreadServer = {
    ...
    title: function (thread) {
        ...
        for (key in state) {
            value = state[key];
            html += buildToast(String.format("%s:%s(%s%)", THREAD_STATE[key], value, parseInt(value / tmp)))
        }
        return html
    },
    ...
    loadData: function (app) {
        $.get(ldThUrl, function (data) {
            ...
            for (i in threads) {
                ...
                html += buildColumn(2, '<button onclick="ThreadServer.dumpThead(this,' + thInfo.id + ')">Dump</button>');
                html += "</div>"
            }
            $("#tableBody").html(html);
